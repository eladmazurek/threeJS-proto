  ...
  if (type === 'init') {
    ...
    for (let i = 0; i < satellites.length; i++) {
      ...
      if (positionAndVelocity.position && typeof positionAndVelocity.position !== 'boolean') {
        ...
        // Calculate heading periodically (staggered) to save performance
        // Update 1/60th of satellites per frame (approx once per second per sat)
        if (sat.heading === undefined || (i % 60 === frameCount % 60)) {
